...
fn test_sops_error_classification_unknown() {
    ...
    let reason = classify_sops_error("some random error message", Some(1)); // Exit code 1 = generic
    // Exit code 1 doesn't map to a specific reason, so it falls through to message parsing
    // If message doesn't match any pattern, it should be Unknown
    // But let's test with a truly unknown message
    let reason = classify_sops_error("completely unknown error", Some(99)); // Unknown exit code
    ...
    let _ = reason; // Just ensure it doesn't panic
}
...
fn test_sops_error_transient_vs_permanent() {
    ...
    for reason in permanent_reasons {
        let error = SopsDecryptionError::new(reason.clone(), "test error".to_string());
        assert!(
            !error.is_transient(),
    ...
    let transient_reasons = vec![
        SopsDecryptionFailureReason::Timeout,
        SopsDecryptionFailureReason::NetworkError,
    ];
