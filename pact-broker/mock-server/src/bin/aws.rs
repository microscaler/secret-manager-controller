...
async fn handle_aws_request(
    State(app_state): State<AwsAppState>,
    request: Request,
) -> Response {
    ...
    match target.as_str() {
        "secretsmanager.CreateSecret" => {
            ...
            let created_date = current_version.as_ref()
                .map(|v| format_timestamp_aws(v.created_at))
                .unwrap_or_else(|| format_timestamp_aws(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs()));
        ...
        "secretsmanager.GetSecretValue" => {
            ...
            let version_id = body_json.as_ref()
            ...
            let created_date = version.as_ref()
                .map(|v| format_timestamp_aws(v.created_at))
                .unwrap_or_else(|| format_timestamp_aws(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs()));
            
            let secret_value = version.as_ref()
                .and_then(|v| v.data.get("SecretString"))
                .and_then(|v| v.as_str())
                .unwrap_or("mock-secret-value");
            
            let version_id_used = version.as_ref()
                .map(|v| v.version_id.clone())
                .unwrap_or_else(|| "unknown".to_string());
            
            // Get staging labels to determine VersionStages
            let labels = app_state.secrets.get_staging_labels(&secret_name).await.unwrap_or_default();
        ...
        "secretsmanager.DeleteSecret" => {
            ...
            let recovery_window_days = body_json.as_ref()
                .and_then(|json| json.get("RecoveryWindowInDays"))
                .and_then(|v| v.as_u64())
                .map(|v| v as u32);
            
            if app_state.secrets.delete_secret_with_recovery(&secret_name, recovery_window_days).await {
                let deletion_date = format_timestamp_aws(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs());
                (
                    StatusCode::OK,
                    Json(json!({
                        "ARN": format!("arn:aws:secretsmanager:us-east-1:123456789012:secret:{}", secret_name),
        ...
        "secretsmanager.UpdateSecretVersionStage" => {
            ...
            let version_id = body_json.as_ref()
                .and_then(|json| json.get("VersionId"))
                .and_then(|v| v.as_str());
            let remove_from_version_id = body_json.as_ref()
                .and_then(|json| json.get("RemoveFromVersionId"))
                .and_then(|v| v.as_str());
            let move_to_version_id = body_json.as_ref()
                .and_then(|json| json.get("MoveToVersionId"))
                .and_then(|v| v.as_str());
            
            if version_id.is_none() || (remove_from_version_id.is_none() && move_to_version_id.is_none()) {
                return aws_error_response(
                    StatusCode::BAD_REQUEST,
                    aws_error_types::INVALID_PARAMETER,
                    "VersionId and either RemoveFromVersionId or MoveToVersionId are required".to_string(),
                );
            }
            ...
            if let (Some(remove_vid), Some(move_vid)) = (remove_from_version_id, move_to_version_id) {
                ...
                if !app_state.secrets.update_staging_label(&secret_name, label, Some(remove_vid), move_vid).await {
                    return aws_error_response(
                        StatusCode::NOT_FOUND,
                        aws_error_types::RESOURCE_NOT_FOUND,
                        format!("One or more versions not found: {} or {}", remove_vid, move_vid),
                    );
                }
            } else if let Some(move_vid) = move_to_version_id {
                // If only MoveToVersionId is provided, just add the label (default to AWSCURRENT)
                let label = "AWSCURRENT";
                if !app_state.secrets.update_staging_label(&secret_name, label, None, move_vid).await {
                    return aws_error_response(
                        StatusCode::NOT_FOUND,
                        aws_error_types::RESOURCE_NOT_FOUND,
