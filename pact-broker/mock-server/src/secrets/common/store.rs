...
impl SecretStore {
    ...
    pub async fn add_version<F>(&self, key: String, version_data: Value, version_id: Option<String>, version_id_generator: F) -> String
    where
        F: FnOnce(&HashMap<String, SecretEntry>, &str) -> String,
    {
        ...
        let new_version_id = version_id.unwrap_or_else(|| {
            version_id_generator(&store, &key)
        });
    ...
    pub async fn get_latest(&self, key: &str) -> Option<SecretVersion> {
        ...
        let mut versions: Vec<_> = entry.versions.iter()
            .filter(|v| v.enabled)
            .collect();
    ...
    pub async fn disable_version(&self, key: &str, version_id: &str) -> bool {
        let mut store = self.store.write().await;
        if let Some(entry) = store.get_mut(key) {
            if let Some(version) = entry.versions.iter_mut().find(|v| v.version_id == version_id) {
