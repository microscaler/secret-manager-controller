...
impl AwsSecretStore {
    ...
    pub async fn add_version(&self, secret_name: &str, version_data: Value, version_id: Option<String>) -> String {
        ...
        let secret_labels = labels.entry(secret_name.to_string()).or_insert_with(HashMap::new);
    ...
    pub async fn get_version_by_label(&self, secret_name: &str, label: &str) -> Option<SecretVersion> {
        let labels = self.staging_labels.read().await;
    ...
    pub async fn update_staging_label(
        ...
        if let Some(remove_vid) = remove_from_version_id {
            if self.store.get_version(secret_name, remove_vid).await.is_none() {
                return false;
            }
        }
        if self.store.get_version(secret_name, move_to_version_id).await.is_none() {
            return false;
        }
    ...
    pub async fn delete_secret_with_recovery(&self, secret_name: &str, _recovery_window_days: Option<u32>) -> bool {
