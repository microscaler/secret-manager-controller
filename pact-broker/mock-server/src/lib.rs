    ...
    if let Some(header_value) = request.headers().get("x-rate-limit") {
        if let Ok(header_str) = header_value.to_str() {
            if header_str.to_lowercase() == "true" {
                ...
                let retry_after = request.headers()
                    .get("x-rate-limit-retry-after")
                    .and_then(|v| v.to_str().ok())
                    .and_then(|s| s.parse::<u64>().ok())
                    .unwrap_or(60);

                warn!("Rate limit header detected - returning 429 with retry-after: {}s", retry_after);
...
pub async fn service_unavailable_middleware(
    request: Request,
    next: Next,
) -> Response {
    // Skip health checks
