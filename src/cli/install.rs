...
pub async fn install_command(
    ...
    if dry_run {
        println!("🔍 Dry-run mode: Would install Secret Manager Controller to namespace '{ns}'");
        println!();
        println!("{}", get_combined_manifests(ns)?);
        return Ok(());
    }

    println!("🚀 Installing Secret Manager Controller...");
    ...
    println!();
    println!("✅ Secret Manager Controller installed successfully!");
    println!();
    println!("📋 Next steps:");
...
fn apply_manifest(manifest_yaml: &str, default_namespace: &str) -> Result<()> {
    ...
    let output = Command::new("kubectl")
        .args(&["apply", "-f", temp_path.to_str().unwrap()])
        .output()
