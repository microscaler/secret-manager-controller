...
impl Reconciler {
    ...
    async fn verify_rbac_for_sops_watch(client: &kube::Client) -> Result<()> {
        ...
        let service_accounts: Api<ServiceAccount> =
            Api::namespaced(client.clone(), EXPECTED_NAMESPACE);
    ...
    pub fn start_sops_key_watch(reconciler: Arc<Self>) {
        tokio::spawn(async move {
            ...
            loop {
                match Self::verify_rbac_for_sops_watch(&reconciler.client).await {
                    Ok(_) => {
                    ...
                    Err(e) => {
                        ...
                        if retry_count >= MAX_RETRIES {
                            ...
                            error!(
                                "   2. Verify ClusterRoleBinding exists and binds ServiceAccount:"
                            );
                            ...
                            error!("      - ClusterRoleBinding may have been created after pod started");
                            error!("      - Kubernetes API server cache may need refresh");
                            error!("      - ServiceAccount token may need regeneration");
                            error!(
                                "      Action: Restart the controller pod to pick up RBAC changes"
                            );
        ...
        let response = match client.get(&artifact_url).send().await {
            ...
            Err(e) => {
                ...
                if is_timeout {
                    ...
                    error!(
                        "  2. Check pods: kubectl get pods -n flux-system -l app=source-controller"
                    );
                    error!("  3. Check endpoints: kubectl get endpoints source-controller -n flux-system");
                    ...
                    error!("  2. Verify service exists: kubectl get svc source-controller -n flux-system");
                } else if is_connection {
                    error!("Connection failed - check network policies and service endpoints");
                    error!("Troubleshooting:");
                    error!("  1. Check endpoints: kubectl get endpoints source-controller -n flux-system");
