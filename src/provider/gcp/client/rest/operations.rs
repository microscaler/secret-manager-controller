...
use crate::provider::gcp::client::common::{
    determine_operation_type, format_secret_path, OperationTracker,
};
...
impl SecretManagerProvider for SecretManagerREST {
    ...
    async fn disable_secret(&self, secret_name: &str) -> Result<bool> {
        ...
        // GCP uses POST /v1/projects/{project}/secrets/{secret}:disable
        let response = self
            .make_request("POST", &format!("{}:disable", secret_path), None)
            .send()
            .await
    ...
    async fn enable_secret(&self, secret_name: &str) -> Result<bool> {
        ...
        // GCP uses POST /v1/projects/{project}/secrets/{secret}:enable
        let response = self
            .make_request("POST", &format!("{}:enable", secret_path), None)
            .send()
            .await
