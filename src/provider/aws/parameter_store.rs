...
impl ConfigStoreProvider for AwsParameterStore {
    async fn create_or_update_config(&self, config_key: &str, config_value: &str) -> Result<bool> {
        ...
        if !parameter_exists {
            ...
            metrics::record_secret_operation("aws_parameter_store", "create", start.elapsed().as_secs_f64());
            return Ok(true);
        }
        ...
        if let Some(current) = current_value {
            if current == config_value {
                debug!("AWS Parameter Store parameter {} unchanged, skipping update", parameter_name);
                metrics::record_secret_operation("aws_parameter_store", "no_change", start.elapsed().as_secs_f64());
                return Ok(false);
            }
        }
        ...
        metrics::record_secret_operation("aws_parameter_store", "update", start.elapsed().as_secs_f64());
        Ok(true)
    }
            ...
            .await
        {
            ...
            Err(e) => {
                if e.to_string().contains("ParameterNotFound") {
                    ...
                    Err(anyhow::anyhow!("Failed to get AWS Parameter Store parameter: {e}"))
                }
            }
        }
    }
